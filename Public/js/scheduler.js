function updateClassSchedule() {
	...
	for (let i = 0; i < classList.length; i++) {
		const span = document
			.getElementById(classList[i].id)
			.getElementsByTagName("span")[0];
		!Object.keys(schedule).some((course) => classList[i].id === course)
...
scheduleViewer.addEventListener("click", function (event) {
	...
	if (
		classKeyRegExp.test(event.target.id) === true ||
		selectedCourse === null ||
		event.target.id === "scheduleViewer" ||
		event.target.id === ""
	)
		return;
